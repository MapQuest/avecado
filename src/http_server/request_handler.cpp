...
void request_handler::handle_request(const request& req, reply& rep)
{
  ...
  // check to see if it should be a 500 error
  if (fail_path_ && boost::starts_with(request_path, *fail_path_)) {
    rep = reply::stock_reply(reply::internal_server_error);
    return;
  }

  // check to see if it should be a hard error
  if (abort_path_ && boost::starts_with(request_path, *abort_path_)) {
     rep.status = reply::internal_server_error;
     rep.is_hard_error = true;
     return;
  }
